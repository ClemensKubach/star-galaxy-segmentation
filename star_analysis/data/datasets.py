...
class Sdss(data.Dataset):
    def __init__(
        ...
        if config.data_dir and config.data_dir != SDSSDataProvider.SINGLETON_DOWNLOADER.to:
            downloader = ImageDownloader(
                config.data_dir, max_workers=os.cpu_count(), run=config.run)
        else:
            downloader = None
    ...
    def _get_patch(self, field: tuple[np.ndarray, np.ndarray], patch_idx: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        x_patch, y_patch = self.__extract_patch(
            x, patch_idx), self.__extract_patch(y, patch_idx)
    ...
    def _get_cropped_data(self, field: tuple[np.ndarray, np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
        ...
        cropping_width_slice = slice(
            self.inner_padding, self.inner_padding + self.cropped_image_shape[0])
        cropping_height_slice = slice(
            self.inner_padding, self.inner_padding + self.cropped_image_shape[1])

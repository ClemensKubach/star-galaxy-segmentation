...
class ImageDownloader:
    ...
    def download(self, to: str) -> tuple[list[str], list[str]]:
    ...
    def __get_images(self, url: str, folder: str) -> list[str]:
        ...
        loaded_images = self.__download_urls(
            [self.__combine_url(url, i) for i in table_links if i.endswith('.bz2')], folder)

        now_final_images = []
        for non_final_link in [i for i in table_links if not i.endswith('.bz2')]:
